...
public class LibrarianController {
        ...
        switch (operation.toLowerCase()) {
            ...
            case "issuemedia":
                if (params.length > 1 && params[0] instanceof Media && params[1] instanceof User) {
                    ...
                    issueMedia(media, member);
                } else {
                    System.out.println("Invalid parameters for IssueMedia.");
                }
                break;
    ...
    private boolean issueMedia(Media media, User member) {
        if (mediaCollection.contains(media) && !media.isCheckedOut()) {
            Loan loan = new Loan(media, member.getId());
            loanHistory.add(loan);
            media.Checkout();
            System.out.println("Media issued successfully: " + media.getTitle() + " to member ID: " + member.getId());
            return true;
        }
        System.out.println("Failed to issue media: " + media.getTitle());
        return false;
    }

    private void giveLateFee(User member) {
